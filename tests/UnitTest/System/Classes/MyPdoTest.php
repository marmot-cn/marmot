 ...
 * 3.拆除基境(fixture)
 * 
 * 这个类因为pdo类封装的还不够严谨,仅仅测试用于可用即可,稍后会完善该类补全所有功能的测试
 * @author chloroplast
...
class MyPdoTest extends GenericTestsDatabaseTestCase{
    ...
    public function testMyPdoInsert(){
        ...
        //插入数据
        $row = Core::$_dbDriver->insert('pcore_table_a',array('title'=>'titleA3','user'=>'userA3'));
    ...
    public function testMyPdoUpdate(){
        ...
        //更新多条数据
        $row = Core::$_dbDriver->update('pcore_table_a',array('title'=>'titleA0'),array('user'=>'userA2'));
        //确认返回值等于更新的条数
    ...
    public function testMyPdoDelete(){
        //删除两条语句
        $row = Core::$_dbDriver->delete('pcore_table_a',array('user'=>'userA2'));
        ...
        //检索出所有row的总数
        $conn = $this->getConnection()->getConnection();
        $query = $conn->query('SELECT COUNT(*) as count FROM pcore_table_a');
        $results = $query->fetchAll(PDO::FETCH_ASSOC);
        //确认总行数已经减少
        $this->assertEquals(1,$results[0]['count'],'after delete the left data size of date not right');

        //根据检索条件搜索确认数据已经删除成功
        $query = $conn->query('SELECT COUNT(*) as count FROM pcore_table_a WHERE user=\'userA2\'');
        $results = $query->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function testMyPdoTransactionRollBackWhenSqlFail(){
        ...
        Core::$_dbDriver->beginTA();
        //插入一条语句
        $result = Core::$_dbDriver->insert('pcore_table_a',array('title'=>'titleA3','user'=>'userA3'));
        $this->assertEquals(1,$result);

        //插入一条语句,这字段名错误
        $result = Core::$_dbDriver->insert('pcore_table_a',array('titleNotExist'=>'titleA4','user'=>'userA4'));
