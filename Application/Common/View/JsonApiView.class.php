...
trait JsonApiView {
	...
	{	
        ...
        $this->links  = [
	            Link::FIRST => new Link($_SERVER['HTTP_HOST'].'/'.$url.'/?page[number]=1&page[size]='.$perpage.$url, null, true),
	            Link::LAST  => new Link($_SERVER['HTTP_HOST'].'/'.$url.'/?page[number]='.$pages.'&page[size]='.$perpage.$url, null, true),
	            Link::PREV  => new Link($_SERVER['HTTP_HOST'].'/'.$url.'/?page[number]='.$prev.'&page[size]='.$perpage.$url, null, true),
	            Link::NEXT  => new Link($_SERVER['HTTP_HOST'].'/'.$url.'/?page[number]='.$next.'&page[size]='.$perpage.$url, null, true),
        ];
		return $this;
	}
	...
	{	
	 	...
	 	$encoder = Encoder::instance($objectsSchema, 
